...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: "Brochure", href: "https://nitj.ac.in/files/1753271909876-Flyer_TFGET26.pdf", section: null, isRoute: false },
  ];
  ...
  return (
    <nav className={`fixed w-full py-3 z-50 transition-all duration-500 ease-in-out ${isScrolled ? "backdrop-blur-lg" : ""}`}>
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3 group">
            <img src={logo} alt="logo" className="w-12 h-6" />
            <span className="normal-text font-bold hidden md:inline-block">TFGET2026</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <a href="https://www.nitj.ac.in/events_registration/ic_tfget_2026/login">Register Now</a>
            </Button>
          </div>

          {/* Mobile Menu Button */}
          <div className="md:hidden">
            <button onClick={toggleMenu} className="text-gray-700 hover:text-conf-green-600 transition-colors duration-300">
              <Menu size={24} />
            </button>
          </div>
        </div>
          ...
          <div className="md:hidden mt-6 pb-6 bg-white/95 backdrop-blur-lg rounded-lg shadow-xl border border-gray-200/50 mx-4">
            <div className="px-4 py-2">
              ...
              <div className="border-b border-gray-100">
                <div className="py-3 text text-gray-600 text-center font-medium">Committees</div>
                {committeeLinks.map((committee) => (
                  <Link
                    key={committee.name}
                    to={committee.href}
              ...
              <div className="border-b border-gray-100">
                <div className="py-3 text text-gray-600 text-center font-medium">Guidelines</div>
                {guidelinesLinks.map((guideline) => (
                  <Link
                    key={guideline.name}
                    to={guideline.href}
